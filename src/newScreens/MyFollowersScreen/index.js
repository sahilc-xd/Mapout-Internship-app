...
const MyFollowersScreen = () => {
  ...
  const [getFollowers, {data, isSuccess, isFetching}] = api.useLazyGetFollowersQuery();
  const [
    followUser,
  ...
  useEffect(()=>{
    getFollowers({userId: user?.user_id ,searchKey: debouncedSearchText, page: page})
  },[])

  useEffect(()=>{
    if(isSuccess && !isFetching){
        setFollowersList(data?.userFollowers);
    }
          ...
          <View f={1}>
                {(page ==1 && isFetching) ? <View f={1} ac="center" jc="center"><ActivityIndicator size={'large'} color={'#000'}/></View> : <FlatList
                    ...
                    ListEmptyComponent={()=>{
                        return(
                            <View f={1}>
                                <Text ta="center">No followers found</Text>
                            </View>
                        )
                    }}
